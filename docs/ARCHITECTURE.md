...
- **sales_agent / account_agent** each run a bounded tool-calling loop
  ...
  own disjoint tool set — see [Tool boundary](#tool-boundary-and-mcp)
...
for why it was split and the measured numbers behind that decision, and
the [README's Evaluation section](../README.md#evaluation) for the
headline results including a change that was measured and reverted.
